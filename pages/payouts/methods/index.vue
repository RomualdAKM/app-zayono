<template>
  <div class="page">
    <p class="page-subtitle">{{ $t('methodsPage.payout.subtitleIndex') }}</p>

    <div class="card">
    <div class="card-head">
      <h2 class="card-title">{{ $t('methodsPage.payout.title') }}</h2>
      <Button class="primary-btn" :label="$t('methodsPage.payout.addMethod')" icon="pi pi-plus" @click="router.push('/payouts/methods/list')" />
    </div>

    <div class="filters">
      <span class="search-wrap">
        <AppIcon name="search" />
        <InputText v-model="search" :placeholder="$t('methodsPage.payout.searchPlaceholder')" class="search-input" />
      </span>
      <Select v-model="statusFilter" :options="statusOptions" optionLabel="label" optionValue="value" :placeholder="$t('methodsPage.filters.status')" showClear class="filter-dropdown" />
      <Select v-model="gatewayFilter" :options="gatewayOptions" optionLabel="label" optionValue="value" :placeholder="$t('methodsPage.filters.gateway')" showClear class="filter-dropdown" />
      <Select v-model="countryFilter" :options="countryOptions" optionLabel="label" optionValue="value" :placeholder="$t('methodsPage.filters.country')" showClear class="filter-dropdown" />
    </div>
    ...
    <div v-else-if="methods.length === 0" class="empty-state">
      ...
      <h3 class="empty-title">{{ $t('methodsPage.payout.empty') }}</h3>
      <p class="empty-desc">{{ $t('methodsPage.payout.emptyHint') }}</p>
      <Button class="primary-btn" :label="$t('methodsPage.payout.viewGateways')" icon="pi pi-plus" @click="router.push('/payouts/gateways/list')" />
    </div>

    <div v-else-if="filtered.length === 0" class="empty-state">
      <AppIcon name="inbox" class="empty-icon" />
      <h3 class="empty-title">{{ $t('methodsPage.payout.noMatchFilters') }}</h3>
    </div>
    ...
    <DataTable v-else :value="filtered" class="m-table" stripedRows rowHover>
      ...
      <Column :header="$t('methodsPage.filters.gateway')">
        <template #body="{ data }">
      ...
      <Column :header="$t('methodsPage.columns.status')" style="width: 120px;">
        <template #body="{ data }">
          <ToggleSwitch :modelValue="data.is_active" @update:modelValue="onToggle(data)" />
        </template>
      </Column>
...
const { list, update, toggle } = useMethods('payout')
...
const load = async () => {
  loading.value = true
  try {
    const [methodsRes, configsRes, availRes] = await Promise.all([list(), getConfigs(), getAvailable(), loadOps()])
    ...
    toast.add({ severity: 'error', summary: t('methodsPage.errorSummary'), detail: e?.data?.message || t('methodsPage.payout.loadError'), life: 4000 })
  } finally {
    loading.value = false
  }
}
...
const filtered = computed(() => {
  ...
  if (q) result = result.filter(m => m.operator_name.toLowerCase().includes(q) || m.operator.toLowerCase().includes(q))
...
const onToggle = async (method: MerchantMethod) => {
  ...
  try {
    ...
    method.is_active = !method.is_active
    toast.add({ severity: 'error', summary: t('methodsPage.errorSummary'), detail: e?.data?.message || t('methodsPage.toggleError'), life: 4000 })
  }
}
...
const handleAssign = async (payload: { aggregator_code: string; fallback_aggregator: string | null; fee_percent: number | null }) => {
  ...
  try {
    // For payouts: keep fallback_aggregator null (aggregators don't fail
    // over for transfers) but DO persist fee_percent — the merchant uses
    // it to mirror the aggregator's payout commission on their side.
    const res = await update(editingMethod.value.id, {
      aggregator_code: payload.aggregator_code,
    ...
    toast.add({ severity: 'success', summary: t('methodsPage.updatedSummary'), life: 3000 })
    showEdit.value = false
  } catch (e: any) {
    toast.add({ severity: 'error', summary: t('methodsPage.errorSummary'), detail: e?.data?.message || t('methodsPage.updateError'), life: 4000 })
  } finally {
...
.row-num { font-size: 12px; color: var(--ze-text-disabled); }

.m-cell { display: flex; gap: 10px; align-items: center; }
...
.m-name { display: block; font-size: 13px; font-weight: 500; color: var(--ze-text-body); }
.m-meta { display: block; font-size: 12px; color: var(--ze-text-muted); }
...
.muted { color: var(--ze-text-disabled); }
</style>
