<template>
  <div class="page">
    <p class="page-subtitle">{{ $t('methodsPage.payout.catalogSubtitle') }}</p>

    <div class="card">
    <div class="card-head">
      <h2 class="card-title">{{ $t('methodsPage.payout.catalogTitle') }}</h2>
      <Button class="outline-btn" :label="$t('methodsPage.back')" icon="pi pi-arrow-left" @click="router.push('/payouts/methods')" />
    </div>

    <div class="filters">
      <Select v-model="countryFilter" :options="countryOptions" optionLabel="label" optionValue="value" :placeholder="$t('methodsPage.filters.country2')" showClear class="filter-dropdown" />
      <Select v-model="currencyFilter" :options="currencyOptions" optionLabel="label" optionValue="value" :placeholder="$t('methodsPage.filters.currency')" showClear class="filter-dropdown" />
      <span class="search-wrap">
        <AppIcon name="search" />
        <InputText v-model="search" :placeholder="$t('methodsPage.payout.searchCatalog')" class="search-input" />
      </span>
    </div>
    ...
    <div v-else class="grid">
      <div v-for="op in filtered" :key="op.code" class="m-card">
        ...
        <p class="m-card-desc">
          {{ payoutCapableCount(op) > 1
            ? $t('methodsPage.payout.availableAtPlural', { count: payoutCapableCount(op) })
            : $t('methodsPage.payout.availableAt', { count: payoutCapableCount(op) }) }}
        </p>
    ...
    <Dialog v-model:visible="showNoGwDialog" :modal="true" :style="{ width: '440px' }" :header="$t('methodsPage.noGatewayDialog.title')">
      <div class="warning-box">
        ...
        <i18n-t keypath="methodsPage.noGatewayDialog.bodyPayout" tag="p">
          <template #name><strong>{{ selectedMethod?.name }}</strong></template>
        </i18n-t>
      </div>
      <template #footer>
        <Button :label="$t('methodsPage.noGatewayDialog.close')" severity="secondary" text @click="showNoGwDialog = false" />
        <Button :label="$t('methodsPage.noGatewayDialog.viewGateways')" @click="goToGatewaysList" />
      </template>
    </Dialog>
...
const payoutCapableAggregators = ref<Set<string>>(new Set())
...
const load = async () => {
  ...
  try {
    await loadOps()
    const [methodsRes, configsRes, availRes] = await Promise.all([list(), getConfigs(), getAvailable()])
    configuredCodes.value = (configsRes.data || []).map(c => c.aggregator_code)
    enabledOperatorCodes.value = new Set((methodsRes.data || []).map(m => m.operator))
    payoutCapableAggregators.value = new Set((availRes.data || []).map(a => a.code))
  } catch {
    toast.add({ severity: 'error', summary: t('methodsPage.errorSummary'), detail: t('methodsPage.catalogLoadError'), life: 4000 })
  } finally {
    loading.value = false
  }
}
...
const filtered = computed(() => {
  let result = catalog.value.operators.filter(op => payoutCapableCount(op) > 0)
  const q = search.value.trim().toLowerCase()
  if (q) result = result.filter(o => o.name.toLowerCase().includes(q) || o.code.toLowerCase().includes(q))
...
const payoutCapableCount = (op: BackendOperator) =>
  op.available_aggregators.filter(a => payoutCapableAggregators.value.has(a)).length
...
const connectMethod = (op: BackendOperator) => {
  if (enabledOperatorCodes.value.has(op.code)) {
    router.push('/payouts/methods')
    return
  }
  const compatibles = op.available_aggregators.filter(
    a => configuredCodes.value.includes(a) && payoutCapableAggregators.value.has(a)
  )
...
const goToGatewaysList = () => {
  ...
  router.push('/payouts/gateways/list')
}
</script>
...
.loading-card { border-radius: 8px !important; }
...
.m-card-head { display: flex; gap: 10px; align-items: center; }
...
.m-card-name { font-size: 14px; font-weight: 600; color: var(--ze-text-strong); margin: 0; }
.m-card-meta { font-size: 12px; color: var(--ze-text-muted); }

.m-card-desc { font-size: 12px; color: var(--ze-text-muted); margin: 0; line-height: 1.5; }
...
.status-tag { font-size: 11px !important; }
...
.warning-box i {
  font-size: 14px;
  /* Match the text's `line-height: 1.5` so icon + text top-align perfectly. */
  line-height: 1.5;
...
@media (max-width: 1024px) { .loading, .grid { grid-template-columns: repeat(2, 1fr); } }
@media (max-width: 640px) { .loading, .grid { grid-template-columns: 1fr; } }
</style>
