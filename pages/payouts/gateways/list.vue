<template>
  <div class="page">
    <div class="page-header-row">
      <p class="page-subtitle">{{ $t('payoutsPage.list.catalogSubtitle') }}</p>
      <Button :label="$t('gatewaysPage.back')" icon="pi pi-arrow-left" class="outline-pill" @click="router.push('/payouts/gateways')" />
    </div>
    ...
    <div class="card filters-card">
      <div class="filters">
        ...
        <Select v-model="countryFilter" :options="countryOptions" optionLabel="label" optionValue="value" :placeholder="$t('gatewaysPage.filters.country')" showClear class="filter-dropdown" />
        <Select v-model="currencyFilter" :options="currencyOptions" optionLabel="label" optionValue="value" :placeholder="$t('gatewaysPage.filters.currency')" showClear class="filter-dropdown" />
      </div>
    </div>
...
const { getAvailable } = useAggregatorConfigs('payout')
const { t } = useI18n()
...
const load = async () => {
  ...
  try {
    ...
    toast.add({ severity: 'error', summary: t('gatewaysPage.errorSummary'), detail: t('gatewaysPage.catalogLoadError'), life: 4000 })
  } finally {
    loading.value = false
  }
}
...
const filtered = computed(() => {
  ...
  if (countryFilter.value) result = result.filter(a => a.supported_countries.includes(countryFilter.value!))
  if (currencyFilter.value) result = result.filter(a => a.supported_currencies.includes(currencyFilter.value!))
  return result
})
...
const goConnect = (agg: AvailableAggregator) => {
  router.push(`/payouts/gateways/${agg.code}?connect=1`)
}
