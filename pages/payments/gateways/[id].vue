  ...
  <div class="page">
    ...
    <div class="back-row">
      <Button
        ...
        text
        size="small"
        @click="router.push(connectMode ? '/payments/gateways/list' : '/payments/gateways')"
      />
    </div>
    ...
    <!-- Connect mode (not yet configured in this env) -->
    <GatewayConnectGatewayForm
      ...
      @back="router.push('/payments/gateways/list')"
    />
      ...
      <div class="section">
        ...
        <div v-if="envConfig?.webhook_auto_registered" class="auto-banner">
          ...
          <div>
            ...
            <p class="auto-banner-body" v-html="t('gatewayDetail.webhook.autoBannerBody', { name: aggregator.name })" />
          </div>
        </div>

        <p class="section-desc" v-html="t('gatewayDetail.webhook.desc', { name: aggregator.name })" />
        ...
        <div class="webhook-field">
          ...
          <div class="webhook-info">
            ...
            <div>
              ...
              <p class="webhook-info-body" v-html="t('gatewayDetail.webhook.infoBody', { name: aggregator?.name ?? '' })" />
            </div>
          </div>
          <p class="webhook-help">
            {{ t('gatewayDetail.webhook.help') }}
          </p>
        </div>
      </div>
    ...
    <Dialog v-model:visible="showEdit" :modal="true" :style="{ width: '480px' }" :header="t('gatewayDetail.editDialog.title')">
      <div v-if="aggregator" class="edit-form">
        ...
        <p class="edit-help">
          {{ t('gatewayDetail.editDialog.help', { envLabel: envStore.current === 'live' ? t('gatewayDetail.editDialog.envLive') : t('gatewayDetail.editDialog.envSandbox') }) }}
        </p>
      </div>
      <template #footer>
    ...
    <Dialog v-model:visible="confirmDisconnect" :modal="true" :style="{ width: '420px' }" :header="t('gatewayDetail.disconnectDialog.title')">
      <p v-html="t('gatewayDetail.disconnectDialog.body', { name: aggregator?.name ?? '', envLabel: envStore.current === 'live' ? t('gatewayDetail.info.production') : t('gatewayDetail.info.sandbox') })" />
      <template #footer>
    ...
    <Dialog v-model:visible="confirmRegenerateWebhook" :modal="true" :style="{ width: '440px' }" :header="t('gatewayDetail.regenerateDialog.title')">
      <p class="confirm-text" v-html="t('gatewayDetail.regenerateDialog.body', { name: aggregator?.name ?? '' })" />
      <template #footer>
    ...
    <Dialog v-model:visible="showNewSecretDialog" :modal="true" :style="{ width: '540px' }" :closable="false" :header="t('gatewayDetail.newSecretDialog.title')">
      ...
      <div class="webhook-info" style="margin-top: 16px;">
        ...
        <div>
          ...
          <p class="webhook-info-body" v-html="t('gatewayDetail.newSecretDialog.infoBody')" />
        </div>
      </div>
      <template #footer>
...
const { getAvailable, getConfig, saveConfig, deleteConfig, regenerateWebhook } = useAggregatorConfigs('payment')
...
const load = async () => {
  ...
  try {
    ...
    if (aggregator.value.configured_in_active_env) {
      ...
      envConfig.value =
        (detailRes.data?.environments || []).find(e => e.environment === envStore.current) || null
    } else {
...
// Prefer the per-config URL emitted by the backend (Moneroo-style token).
// Fall back to the legacy /webhooks/{aggregator} shape if no envConfig is loaded yet.
...
const handleConnect = async (credentials: Record<string, string>) => {
  ...
  try {
    const res = await saveConfig({
      aggregator_code: aggregator.value.code,
      credentials,
      is_active: true,
    })
    // If the backend returned the webhook secret in clear (first save), surface it
    const secret = (res as any)?.data?.webhook_secret
    if (secret) {
      newWebhookSecret.value = secret
      showNewSecretDialog.value = true
    } else {
      toast.add({ severity: 'success', summary: t('gatewayDetail.connectSuccess.summary'), detail: t('gatewayDetail.connectSuccess.detail'), life: 3500 })
    }
    // Drop the ?connect=1 query and reload
    router.replace(`/payments/gateways/${code.value}`).then(load)
  } catch (e: any) {
    // Surface validation/auth errors persistently inside the form
...
const handleRegenerateWebhook = async () => {
  ...
  try {
    ...
    showNewSecretDialog.value = true
    await load() // refresh masked secret + new URL
  } catch (e: any) {
...
.webhook-info i {
  font-size: 14px;
  /* Match text `line-height: 1.5` so icon + first text line align visually. */
  line-height: 1.5;
  color: var(--ze-brand);
  flex-shrink: 0;
}
...
.danger {
  border-color: var(--ze-danger-border);
}
