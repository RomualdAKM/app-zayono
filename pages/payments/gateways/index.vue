<template>
  <div class="page">
    <div class="page-header-row">
      <p class="page-subtitle">{{ $t('gatewaysPage.subtitleIndex') }}</p>
    ...
    <div v-else-if="rows.length === 0" class="card empty-state">
      ...
      <p class="empty-title">{{ $t('gatewaysPage.empty') }}</p>
      <p class="empty-desc">{{ $t('gatewaysPage.emptyHintFirst') }}</p>
...
const load = async () => {
  ...
  try {
    ...
    toast.add({ severity: 'error', summary: t('status.error'), detail: t('gatewaysPage.loadError'), life: 4000 })
  } finally {
    loading.value = false
  }
}
...
const goToCatalog = () => router.push('/payments/gateways/list')
const goToDetail = (code: string) => router.push(`/payments/gateways/${code}`)
