<template>
  <div class="page">
    <p class="page-subtitle">{{ $t('statisticsPage.subtitle.payouts') }}</p>
    ...
    <template v-else>
      <div class="kpi-grid">
        <DashboardKpiCard
          tone="blue"
          icon="pi-list"
          :label="$t('statisticsPage.kpi.totalPayouts')"
      ...
      <div class="section">
        <div class="section-head">
          <h3 class="section-title">{{ $t('statisticsPage.volumeByCurrency') }}</h3>
          <p class="section-subtitle">{{ $t('statisticsPage.volumeByCurrencyHint') }}</p>
        </div>

        <div v-if="!revenue.length" class="empty-state">
          <AppIcon name="inbox" class="empty-icon" />
          <p class="empty-text">{{ $t('statisticsPage.noPayoutPeriod') }}</p>
        </div>
...
const queryParams = () => {
  const params: Record<string, any> = { type: 'payout' }
...
.loading-card { border-radius: 10px !important; }
