<template>
  ...
  <div class="ck-card">
    ...
    <template v-if="step === 1">
      ...
      <div v-if="!operatorGroups.length" class="ck-empty">
      ...
      <div v-for="group in operatorGroups" :key="group.key" class="ck-section">
        ...
        <div class="ck-methods">
          <CheckoutMethodCard
            ...
            :key="op.code"
            :name="op.name"
            :subtitle="fxUnavailableLabel(op) ?? (group.key === 'international' ? undefined : countryLabel(op.country))"
...
// Non-empty groups for rendering: Mobile Money (country-gated) then
// International. The momo group keeps the generic label when it is the only
// group and switches to "Mobile Money" once an International group is present.
